export type CameraController = {
  ...
  captureFrameBlob: (quality?: number) => Promise<Blob>;
  hasStream: () => boolean;
  getVideoSize: () => { width: number; height: number };
};

export function createCameraController(video: HTMLVideoElement): CameraController {
  ...
  async function captureFrameBlob(quality = 0.7): Promise<Blob> {
